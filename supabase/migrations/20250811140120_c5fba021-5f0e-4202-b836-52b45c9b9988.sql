-- 1) Create public bucket for processed certificates
insert into storage.buckets (id, name, public)
values ('certificados', 'certificados', true)
on conflict (id) do nothing;

-- 2) Allow public read access to objects in the certificados bucket
create policy if not exists "Certificados are publicly readable"
on storage.objects
for select
using (bucket_id = 'certificados');

-- 3) Allow authenticated users to upload into the certificados bucket
...
-- 4) Update validation function: remove requirement for link when setting Procesada
